...
- `kiro_local`: runs your local `kiro-cli` CLI
- `process`: generic shell command adapter
- `http`: calls an external HTTP endpoint

For `claude_local`, `codex_local`, and `kiro_local`, Paperclip assumes the CLI is already installed and authenticated on the host machine.
...
1. Check adapter command availability (`claude`/`codex`/`kiro-cli` installed and logged in).
...
1. Choose adapter (`claude_local`, `codex_local`, or `kiro_local`).
